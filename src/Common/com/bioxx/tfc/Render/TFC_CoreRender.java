...
public class TFC_CoreRender
{
	...
	public static boolean renderWoodTrunk(Block block, int i, int j, int k, RenderBlocks renderblocks)
	{
		...
		/*if(blockAccess.getBlockMaterial(i, j+1, k) == Material.leaves || blockAccess.getBlockMaterial(i, j-1, k) == Material.leaves || 
                ...
                TileEntity ate = blockAccess.getTileEntity(i, j, k);
                Block ablock = blockAccess.getBlock(i, j-1, k);
		if(ate != null && (ablock == TFCBlocks.fruitTreeWood || ablock.isOpaqueCube()))//blockAccess.getBlock(i, j-1, k);
		{
			renderblocks.setRenderBounds(0.3F, 0.0F, 0.3F, 0.7F, 1.0F, 0.7F);
			renderblocks.renderStandardBlock(block, i, j, k);
		}
                ablock = blockAccess.getBlock(i-1, j, k);
		if(ablock.getMaterial() == Material.leaves || ablock == TFCBlocks.fruitTreeWood)//blockAccess.getBlock(i-1, j, k)
		{
		...
		if(ablock.getMaterial() == Material.leaves || ablock == TFCBlocks.fruitTreeWood)//blockAccess.getBlock(i+1, j, k)
		{
			...
			renderblocks.renderStandardBlock(block, i, j, k);
                ...
                ablock = blockAccess.getBlock(i, j, k-1);
		if(ablock.getMaterial() == Material.leaves || ablock == TFCBlocks.fruitTreeWood)//blockAccess.getBlock(i, j, k-1)
		{
		...
		if(ablock.getMaterial() == Material.leaves || ablock == TFCBlocks.fruitTreeWood)//blockAccess.getBlock(i, j, k+1)
		{
			...
			renderblocks.renderStandardBlock(block, i, j, k);
                ...
                ablock = blockAccess.getBlock(i, j-1, k);
		if(!((TEFruitTreeWood)ate/*blockAccess.getTileEntity(i, j, k)*/).isTrunk && ablock != TFCBlocks.fruitTreeWood && !ablock.isOpaqueCube())//blockAccess.getBlock(i, j-1, k)
		{
			renderblocks.setRenderBounds(0.0F, 0.4F, 0.4F, 0.5F, 0.6F, 0.6F);
			renderblocks.renderStandardBlock(block, i, j, k);
